...
export class WidgetsView extends LitElement {
  ...
  private renderWidgetContent(widget: Widget) {
    switch (widget.type) {
      case 'clock':
        return html`
  ...
  private renderWidgetLibrary() {
    ...
    return html`
      <div class="widget-library" @click="${this.closeWidgetLibrary}">
        <div class="widget-library-dialog" @click="${(e: Event) => e.stopPropagation()}">
  ...
  private openWidgetLibrary(): void {
    this.showLibrary = true;
  }

  private closeWidgetLibrary(): void {
    this.showLibrary = false;
  }

  private addWidget(widgetTemplate: WidgetTemplate): void {
  ...
  private handleWidgetRefresh(e: WidgetEvent): void {
    const widgetId = e.detail.widgetId;
    console.log('Refreshing widget:', widgetId);

    // Find and refresh the specific widget
    const widgetElement = this.shadowRoot?.querySelector(`dashboard-widget[widgetId="${widgetId}"]`) as HTMLElement & { refresh?: () => void };
    if (widgetElement && widgetElement.refresh) {
      widgetElement.refresh();
    }
  }

  private handleWidgetRemove(e: WidgetEvent): void {
    const widgetId = e.detail.widgetId;
    this.widgets = this.widgets.filter(w => w.id !== widgetId);
    this.saveWidgets();
  }

  private handleWidgetConfigure(e: WidgetEvent): void {
    const widgetId = e.detail.widgetId;
